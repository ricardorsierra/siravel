...
class SiravelService
{
    ...
    {
        $this->imageRepo = App::make('Sitec\Siravel\Repositories\ImageRepository');
    }
    ...
    public function asset($path, $contentType = 'null', $fullURL = true)
    {
        ...
        return url('siravel/asset/'.CryptoServiceFacade::url_encode($path).'/'.CryptoServiceFacade::url_encode($contentType));
    }
    ...
    public function editBtn($type = null, $id = null)
    {
        if (Gate::allows('siravel', Auth::user())) {
            if (!is_null($id)) {
                return '<a href="'.url('siravel/'.$type.'/'.$id.'/edit').'" class="btn btn-xs btn-default pull-right"><span class="fa fa-pencil"></span> Edit</a>';
            } else {
                return '<a href="'.url('siravel/'.$type).'" class="btn btn-xs btn-default pull-right"><span class="fa fa-pencil"></span> Edit</a>';
            }
        }
    ...
    public function rollbackUrl($object)
    {
        ...
        return url('siravel/rollback/'.$class.'/'.$object->id);
    }
