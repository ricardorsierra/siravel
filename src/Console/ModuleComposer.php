...
class ModuleComposer extends Command
{
     ...
     * @var string
     */
    protected $description = 'Convert a Siravel module to a composer package';
    ...
    {
        $moduleDir = base_path(config('siravel.module-directory')).'/'.ucfirst($this->argument('module'));
        if (is_dir($moduleDir)) {
    ...
    public function updateFile($file, $namespace, $module)
    {
        ...
        $contents = str_replace('Siravel\\Modules', $provider[0], $contents);
