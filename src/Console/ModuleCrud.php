...
use Sitec\CrudMaker\Generators\CrudGenerator;
...
class ModuleCrud extends Command
{
     ...
     * @var string
     */
    protected $description = 'Generate a CRUD module for Siravel';
    ...
    public function handle()
    {
        ...
        $moduleDirectory = base_path('siravel/modules/'.ucfirst(str_plural($this->table)));
        ...
        if (!is_dir($moduleDirectory)) {
            ...
            mkdir($moduleDirectory.'/Publishes/app/Http/Controllers/Siravel', 0777, true);
        ...
        file_put_contents($moduleDirectory.'/Views/menu.blade.php', "<li class=\"@if (Request::is('siravel/".strtolower(str_plural($this->table))."') || Request::is('siravel/".strtolower(str_plural($this->table))."/*')) active @endif\"><a href=\"{{ url('siravel/".strtolower(str_plural($this->table))."') }}\"><span class=\"fa fa-file\"></span> ".ucfirst(str_plural($this->table)).'</a></li>');
        ...
        $config = [
            ...
            'routes_prefix' => "<?php \n\nRoute::group(['namespace' => 'Siravel\Modules\\".ucfirst(str_plural($this->table))."\Controllers', 'prefix' => 'siravel', 'middleware' => ['web', 'auth', 'siravel']], function () { \n\n",
            'routes_suffix' => "\n\n});",
            '_app_namespace_' => app()->getInstance()->getNamespace(),
            '_namespace_services_' => 'Siravel\Modules\\'.ucfirst(str_plural($this->table)).'\Services',
            '_namespace_facade_' => 'Siravel\Modules\\'.ucfirst(str_plural($this->table)).'\Facades',
            '_namespace_model_' => 'Siravel\Modules\\'.ucfirst(str_plural($this->table)).'\Models',
            '_namespace_controller_' => 'Siravel\Modules\\'.ucfirst(str_plural($this->table)).'\Controllers',
            '_namespace_request_' => 'Siravel\Modules\\'.ucfirst(str_plural($this->table)).'\Requests',
        ...
        $appConfig['_path_controller_'] = $moduleDirectory.'/Publishes/app/Http/Controllers/Siravel';
        ...
        $appConfig['_namespace_controller_'] = $config['_app_namespace_'].'Http\Controllers\Siravel';
        $appConfig['routes_prefix'] = "<?php \n\nRoute::group(['namespace' => 'Siravel', 'middleware' => ['web']], function () {\n\n";
        ...
        Artisan::call('make:migration', [
            ...
            '--path' => 'siravel/modules/'.ucfirst(str_plural($this->table)).'/Publishes/database/migrations',
    ...
    {
        if (!is_dir(base_path('siravel'))) {
            @mkdir(base_path('siravel'));
        }

        if (!is_dir(base_path('siravel/modules'))) {
            mkdir(base_path('siravel/modules'));
        }
    }
    ...
    public function setSchema()
    {
        if ($this->option('schema')) {
            $migrationFiles = $this->filesystem->allFiles(base_path('siravel/modules/'.ucfirst(str_plural($this->table)).'/Publishes/database/migrations'));
