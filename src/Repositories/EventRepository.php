...
class EventRepository
{
    ...
    {
        return Event::where('is_published', 1)->where('published_at', '<=', Carbon::now()->format('Y-m-d h:i:s'))->orderBy('created_at', 'desc')->paginate(Config::get('sicms.pagination', 25));
    }
    ...
    public function update($event, $payload)
    {
        if (!empty($payload['lang']) && $payload['lang'] !== config('sicms.default-language', 'en')) {
            return $this->translationRepo->createOrUpdate($event->id, 'Sitec\Siravel\Models\Event', $payload['lang'], $payload);
        } else {
